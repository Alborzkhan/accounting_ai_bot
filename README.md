...
- حساب کاوه‌نگار (یا هر سرویس پیامک دیگر) برای ارسال کد تایید ورود در محیط Production
...
سپس مقادیر `.env` را تکمیل کنید (توکن تلگرام/بله، کلید OpenAI، کلید کاوه‌نگار و ...). فایل `.env` هرگز نباید به گیت اضافه شود (در `.gitignore` پوشش داده شده).
...
ورود کاربران در وب‌اپ از طریق کد تایید یک‌بارمصرف (OTP) ارسالی با پیامک انجام می‌شود (`core/auth.py`, `core/sms_service.py`). اگر `KAVENEGAR_API_KEY` در `.env` خالی باشد، کد تایید فقط در `logs/app.log` ثبت می‌شود — این حالت فقط برای توسعه/تست است و **قبل از انتشار عمومی باید کلید واقعی پیامک تنظیم شود**.
